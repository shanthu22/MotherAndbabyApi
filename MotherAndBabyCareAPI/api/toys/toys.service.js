...
export const serviceAddToys = async (data, callback) => {
  try {
    const results = await pool.query(
      `INSERT INTO toys (name, description, price, quantity,  imagePath)VALUES ("${data.name}", "${data.description}", ${data.price}, ${data.quantity},"${data.imagePath}");`
    );
...
export const serviceUpdateToys = async (data, callback) => {
  try {
    const results = await pool.query(
      `UPDATE toys SET name ="${data.name}", description = "${data.description}", price = ${data.price}, quantity = ${data.quantity}, imagePath = "${data.imagePath}" WHERE id = ${data.id}; `
    );
    callback(null, results);
  } catch (error) {
    callback(error, null);
